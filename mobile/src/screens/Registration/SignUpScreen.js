...
const SignUpScreen = () => {
    ...
    const isValidForm = () => {
        if(Object.values(validForm).every(val => val == true) && !isFormEmpty()){
        ...
        if(isValidForm()){
            ...
            axios.post(`${BACKENDPOINT}/SignUp`, userInfo).then(res => {
                ...
                if(error == "Username and Email Taken!"){
                    setValidForm(prevState => ({...prevState, usernameNotTaken: false,}));
                    setValidForm(prevState => ({...prevState, emailNotTaken: false,}));
    ...
    const handleValidRepeat = (val) => {
        if(password != repeatPassword){
            setValidForm(prevState => ({...prevState, validRepeatPassword: false}))
        }else{
            setValidForm(prevState => ({...prevState, validRepeatPassword: true}))
        }
    };
    ...
    return (
        <ScrollView showsVerticalScrollIndicator = { false }>
            <View style={styles.root}>
                ...
                {validForm.validUsername ? 
                    null : 
                    <Text style= {styles.error}>Username must be at least 4 characters long!</Text>}
                {validForm.usernameNotTaken ? 
                    null : 
                ...
                {validForm.emailNotTaken ? 
                    null: 
                    <Text style={styles.error}>Email already in use with another account!</Text>}
                ...
                {validForm.validPassword ? 
                    null : 
                    <Text style={styles.error}>Password must be at least 8 characters long!</Text>}

                <CustomInput 
                    placeholder = "Re-enter Password" 
                    value = {repeatPassword} 
                    setValue = {setRepeatPassword} 
                    secureTextEntry autoCapitalize={false} 
                    onEndEditing={(e)=>handleValidRepeat(e.nativeEvent.text)}
                />
                {validForm.validRepeatPassword ? 
                    null :
                ...
                <CustomButton text= "Have an account? Log In!" onPress= { onLogIn } type="TERTIARY" ></CustomButton>
            </View>
        </ScrollView>
    );
};
